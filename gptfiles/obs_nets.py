...
from robomimic.models.base_nets import Module, GPT, GPTConfig, Sequential, MLP, RNN_Base, ResNet18Conv, SpatialSoftmax, \
...
class RNN_MIMO_MLP(Module):
    ...
    def forward(self, rnn_init_state=None, return_state=False, **inputs):
        ...
        # print(self.nets["decoder"].output_shape())
...
class GPT_MIMO_MLP(Module):
    ...
    def __init__(
        ...
        Args:
            ...
            encoder_kwargs (dict or None): If None, results in default encoder_kwargs being applied. Otherwise, should
                ...
                obs_modality2: dict
                    ...
        """
        super(GPT_MIMO_MLP, self).__init__()
        assert isinstance(input_obs_group_shapes, OrderedDict)
        assert np.all([isinstance(input_obs_group_shapes[k], OrderedDict) for k in input_obs_group_shapes])
        assert isinstance(output_shapes, OrderedDict)
        self.input_obs_group_shapes = input_obs_group_shapes
        ...
        if self._has_mlp:
            ...
            self.nets["decoder"] = ObservationDecoder(
                decode_shapes=self.output_shapes,
                input_feat_dim=vocab_size,
            )
            if self.per_step:
                per_step_net = self.nets["decoder"]

        # set configs for core GPT network
        config = GPTConfig(vocab_size, block_size, input_size = rnn_input_dim, n_embd = n_embd, n_layer = n_layer, n_head = n_head)

        
        self.nets["gpt"] = GPT(config, per_step_net=per_step_net)
    ...
    #             depending on the RNN type
    #     """
    #     return self.nets["gpt"].get_rnn_init_state(batch_size, device=device)
    ...
    def output_shape(self, input_shape):
        ...
        T = input_shape[obs_group][mod][0]
        TensorUtils.assert_size_at_dim(input_shape, size=T, dim=0, 
                msg="GPT_MIMO_MLP: input_shape inconsistent in temporal dimension")
        # returns a dictionary instead of list since outputs are dictionaries
        return { k : [T] + list(self.output_shapes[k]) for k in self.output_shapes }

    def forward(self, return_state=False, **inputs):
        """
        Args:
            inputs (dict): a dictionary of dictionaries with one dictionary per
                observation group. Each observation group's dictionary should map
        ...
        # use encoder to extract flat rnn inputs
        gpt_inputs = TensorUtils.time_distributed(inputs, self.nets["encoder"], inputs_as_kwargs=True)

        #changing to idx format for GPT, don't understand what to do with d
        #b, t, d = rnn_inputs
        #idx = 

        assert gpt_inputs.ndim == 3  # [B, T, D]
        if self.per_step:
            return self.nets["gpt"].forward(idx=gpt_inputs)
        
        # apply MLP + decoder to last RNN output
        outputs, loss = self.nets["gpt"].forward(idx=gpt_inputs)
    ...
    def __repr__(self):
        ...
        msg += textwrap.indent("\n" + self._to_string(), indent)
        msg += textwrap.indent("\n\nencoder={}".format(self.nets["encoder"]), indent)
        msg += textwrap.indent("\n\nrnn={}".format(self.nets["gpt"]), indent)
        msg = header + '(' + msg + '\n)'
        return msg
