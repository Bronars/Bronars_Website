...
from robomimic.models.obs_nets import MIMO_MLP, RNN_MIMO_MLP, GPT_MIMO_MLP#, RNN_MIMO_MLP_ORIGINAL
...
class GPTActorNetwork(GPT_MIMO_MLP):
    """
    An GPT policy network that predicts actions from observations.
    """
    def __init__(
        self,
        obs_shapes,
        ...
        output_shapes = self._get_output_shapes()
        super(GPTActorNetwork, self).__init__(
    ...
    def output_shape(self, input_shape):
        ...
        T = input_shape[mod][0]
        TensorUtils.assert_size_at_dim(input_shape, size=T, dim=0, 
                msg="GPTActorNetwork: input_shape inconsistent in temporal dimension")
        return [T, self.ac_dim]

    def forward(self, obs_dict, goal_dict=None, return_state=False):
        """
        Forward a sequence of inputs through the RNN and the per-step network.
        ...
        outputs = super(GPTActorNetwork, self).forward(
            obs=obs_dict, goal=goal_dict, return_state=return_state)
    ...
    def forward_step(self, obs_dict, goal_dict=None):
        ...
        obs_dict = TensorUtils.to_sequence(obs_dict)
        action = self.forward(
            obs_dict, goal_dict, return_state=True)
        return action[:, 0]
...
class GPTGMMActorNetwork(GPTActorNetwork):
    """
    ...
    def __init__(
        ...
        super(GPTGMMActorNetwork, self).__init__(
            obs_shapes=obs_shapes,
            ac_dim=ac_dim,
            mlp_layer_dims=mlp_layer_dims,
            vocab_size = vocab_size,
