...
def algo_config_to_class(algo_config):
    ...
    gpt_enabled = ("gpt" in algo_config and algo_config.gpt.enabled)

    assert sum([rnn_enabled, gpt_enabled]) <= 1
    if algo_config.rnn.enabled:
        if gmm_enabled:
            return BC_RNN_GMM, {}
        return BC_RNN, {}
    if algo_config.gpt.enabled:
        #print("GPT")
        if gmm_enabled:
            return BC_GPT_GMM, {}
        return BC_GPT, {}
...
class BC_GPT(BC):
    """
    BC training with an GPT policy.
    """
    def _create_networks(self):
        """
        Creates networks and places them into @self.nets.
        """
        self.nets = nn.ModuleDict()
        self.nets["policy"] = PolicyNets.GPTActorNetwork(
            obs_shapes=self.obs_shapes,
            goal_shapes=self.goal_shapes,
            ac_dim=self.ac_dim,
            mlp_layer_dims=self.algo_config.actor_layer_dims,
            encoder_kwargs=ObsUtils.obs_encoder_kwargs_from_config(self.obs_config.encoder),
            **BaseNets.gpt_args_from_config(self.algo_config.gpt),
        )

        self._obs_buffer = None
        #self._gpt_horizon = self.algo_config.gpt.horizon
        self._gpt_counter = 0
        self._gpt_is_open_loop = self.algo_config.gpt.get("open_loop", False)
    ...
    def get_action(self, obs_dict, goal_dict=None):
        ...
        if self._obs_buffer is None:
            # batch_size = list(obs_dict.values())[0].shape[0]
            # self._rnn_hidden_state = self.nets["policy"].get_rnn_init_state(batch_size=batch_size, device=self.device)
            self._obs_buffer = { k : obs_dict[k].repeat(10, 1) for k in TensorUtils.clone(obs_dict)}#TensorUtils.to_sequence(TensorUtils.clone(TensorUtils.detach(obs_dict)))
        ...
        self._obs_buffer = { k : torch.cat((self._obs_buffer[k][1:], TensorUtils.clone(obs_dict[k])), axis = 0) for k in self._obs_buffer}

        #from IPython import embed; embed()

        obs_to_use = { k : self._obs_buffer[k][None, :] for k in self._obs_buffer}
        ...
        if self._gpt_is_open_loop:
            # replace current obs with last recorded obs
            obs_to_use = self._open_loop_obs
            self._obs_buffer = self._open_loop_obs

        self._gpt_counter += 1
        action = self.nets["policy"].forward(
            obs_to_use, goal_dict=goal_dict)

        action1 = action[:, -1]

        #action = action[0, 0, :].repeat(1, 1)
        #from IPython import embed; embed()
        return action1
        ...
        # if self._rnn_hidden_state is None or self._rnn_counter % self._rnn_horizon == 0:
        #     batch_size = list(obs_dict.values())[0].shape[0]
        #     self._rnn_hidden_state = None #self.nets["policy"].get_rnn_init_state(batch_size=batch_size, device=self.device)
        ...
        # self._gpt_counter += 1
        # action = self.nets["policy"].forward_step(
        #     obs_to_use, goal_dict=goal_dict)#, rnn_state=self._rnn_hidden_state)
        # return action
...
class BC_GPT_GMM(BC_GPT):
    ...
    def _create_networks(self):
        ...
        """
        assert self.algo_config.gmm.enabled
        assert self.algo_config.gpt.enabled

        self.nets = nn.ModuleDict()
        self.nets["policy"] = PolicyNets.GPTGMMActorNetwork(
            obs_shapes=self.obs_shapes,
            goal_shapes=self.goal_shapes,
            ac_dim=self.ac_dim,
            mlp_layer_dims=self.algo_config.actor_layer_dims,
