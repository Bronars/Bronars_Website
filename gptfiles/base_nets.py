...
def gpt_args_from_config(gpt_config):
    """
    Takes a Config object corresponding to RNN settings
    (for example `config.algo.rnn` in BCConfig) and extracts
    rnn kwargs for instantiating rnn networks.
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        #Testing Per Step Net
        if self.per_step_net is not None:
            #print("no we actually here")
            #print(lala.sdokn)
            logits = TensorUtils.time_distributed(logits, self.per_step_net)

        #from IPython import embed; embed()
        return logits

### End code from minGPT
